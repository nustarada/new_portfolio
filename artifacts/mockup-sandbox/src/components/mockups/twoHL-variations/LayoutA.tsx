...
const cs = {
  title: "2Hour Learning",
  sub: "B2B EdTech platform",
  role: "UX Designer",
  timeline: "3 weeks",
  sections: [
...
function Defs() {
  return (
    <defs>
      <filter id="wf-2a-sk">
        <feTurbulence type="turbulence" baseFrequency="0.018" numOctaves="3" seed="7" result="noise"/>
        <feDisplacementMap in="SourceGraphic" in2="noise" scale="1.8" xChannelSelector="R" yChannelSelector="G"/>
      </filter>
      <pattern id="wf-2a-grid" width="24" height="24" patternUnits="userSpaceOnUse">
        <path d="M24 0L0 0 0 24" fill="none" stroke="#ddd5c5" strokeWidth="0.5"/>
      </pattern>
      <pattern id="wf-2a-hatch" width="6" height="6" patternUnits="userSpaceOnUse" patternTransform="rotate(45)">
...
const Ln = ({ x, y, w, opacity = 1 }: any) => <rect x={x} y={y} width={w} height={7} rx="1" fill="#d1cfc8" opacity={opacity}/>;
const Xbox = ({ x, y, w, h }: any) => (
  <g><rect x={x} y={y} width={w} height={h} fill="#f0ede8" stroke="#b8b0a4" strokeWidth="1.2" rx="1"/>
  <line x1={x} y1={y} x2={x+w} y2={y+h} stroke="#b8b0a4" strokeWidth="0.9"/>
  <line x1={x+w} y1={y} x2={x} y2={y+h} stroke="#b8b0a4" strokeWidth="0.9"/></g>
);
const Pill = ({ x, y, w = 80 }: any) => (
  <g><rect x={x} y={y} width={w} height={18} rx="9" fill="#e8e4de" stroke="#b8b0a4" strokeWidth="0.9"/>
  <rect x={x+8} y={y+6} width={w-16} height={6} rx="1" fill="#b8b0a4" opacity="0.6"/></g>
);
  ...
  return (
    <div className="w-full bg-[#fdf8f0]" style={{ fontFamily: "monospace" }}>
      <svg viewBox={`0 0 1280 ${totalH}`} className="w-full h-auto" xmlns="http://www.w3.org/2000/svg">
        ...
        <rect width="1280" height={totalH} fill="url(#wf-2a-grid)" opacity="0.7"/>
        <g filter="url(#wf-2a-sk)">
          {/* NAV */}
          ...
          <rect x={1200} y={18} width={56} height={16} rx="3" fill="url(#wf-2a-hatch)" stroke="#9ca3af" strokeWidth="1"/>
          {/* HERO */}
          ...
          <rect x={CONTENT_X} y={100} width={440} height={22} rx="1" fill="#c8c4bc" opacity="0.8"/>
          <rect x={CONTENT_X} y={128} width={240} height={13} rx="1" fill="#d1cfc8" opacity="0.7"/>
          ...
          {sectionBlocks.map(({ label, content, h, y }) => (
            <g key={label}>
              ...
              {content === "highlight" && (<><rect x={CONTENT_X} y={y+40} width={CONTENT_W} height={52} rx="2" fill="#e8e4de" stroke="#c8c4bc" strokeWidth="1"/><Ln x={CONTENT_X+20} y={y+52} w={700}/><Ln x={CONTENT_X+20} y={y+64} w={480}/><text x={CONTENT_X+20} y={y+86} fontSize="7.5" fill="#9ca3af" fontFamily="monospace">[ key insight ]</text></>)}
              {content === "text+wireframes" && (<><Ln x={CONTENT_X} y={y+44} w={800}/><Ln x={CONTENT_X} y={y+56} w={680}/>{[0,1,2].map(i=><Xbox key={i} x={CONTENT_X+i*388} y={y+74} w={370} h={90}/>)}<text x={CONTENT_X+150} y={y+182} fontSize="7.5" fill="#9ca3af" fontFamily="monospace">option A</text><text x={CONTENT_X+526} y={y+182} fontSize="7.5" fill="#9ca3af" fontFamily="monospace">option B</text><text x={CONTENT_X+910} y={y+182} fontSize="7.5" fill="#9ca3af" fontFamily="monospace">chosen ✓</text></>)}
              {content === "decisions" && (<>{[0,1,2,3].map(i=><g key={i}><Ln x={CONTENT_X} y={y+44+i*36} w={60} opacity={0.5}/><Ln x={CONTENT_X+74} y={y+44+i*36} w={400}/><Ln x={CONTENT_X+74} y={y+56+i*36} w={700}/></g>)}</>)}
              {content === "screens" && (<>{[0,1,2].map(i=><Xbox key={i} x={CONTENT_X+i*388} y={y+40} w={370} h={140}/>)}</>)}
              {content === "findings" && (<>{[0,1,2,3].map(i=><g key={i}><rect x={CONTENT_X} y={y+40+i*26} width={8} height={8} rx="1" fill="#c8c4bc"/><Ln x={CONTENT_X+16} y={y+40+i*26} w={640}/><Ln x={CONTENT_X+16} y={y+52+i*26} w={480} opacity={0.6}/></g>)}</>)}
              {content === "metrics" && (<>{[0,1,2,3].map(i=><g key={i}><rect x={CONTENT_X+i*292} y={y+40} width={276} height={60} rx="2" fill="#f0ede8" stroke="#c8c4bc" strokeWidth="1"/><rect x={CONTENT_X+i*292+16} y={y+52} width={100} height={18} rx="1" fill="#c8c4bc"/><Ln x={CONTENT_X+i*292+16} y={y+78} w={160} opacity={0.5}/></g>)}</>)}
              {content === "text" && (<><Ln x={CONTENT_X} y={y+44} w={860}/><Ln x={CONTENT_X} y={y+56} w={740}/><Ln x={CONTENT_X} y={y+68} w={800}/><Ln x={CONTENT_X} y={y+80} w={600} opacity={0.7}/></>)}
          ...
          <text x={1040} y={totalH-12} fontSize="8" fill="#c8c4bc" fontFamily="monospace">Layout A · Single Column</text>
        </g>
      </svg>
    </div>
  );

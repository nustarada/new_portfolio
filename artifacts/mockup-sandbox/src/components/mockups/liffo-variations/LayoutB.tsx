...
const Hbox = ({ x, y, w, h }: any) => (
  <rect x={x} y={y} width={w} height={h} fill="url(#wf-b-hatch)" stroke="#9ca3af" strokeWidth="1" rx="1"/>
);
    ...
    <div className="w-full bg-[#fdf8f0]">
      <svg viewBox={`0 0 1280 ${TOTAL_H}`} className="w-full h-auto" xmlns="http://www.w3.org/2000/svg">
        <defs>
          ...
          <pattern id="wf-b-grid" width="24" height="24" patternUnits="userSpaceOnUse">
            <path d="M24 0L0 0 0 24" fill="none" stroke="#ddd5c5" strokeWidth="0.5"/>
          </pattern>
          <pattern id="wf-b-hatch" width="6" height="6" patternUnits="userSpaceOnUse" patternTransform="rotate(45)">
            <line x1="0" y1="0" x2="0" y2="6" stroke="#9ca3af" strokeWidth="0.9" opacity="0.55"/>
          </pattern>
        </defs>

        <rect width="1280" height={TOTAL_H} fill="#fdf8f0"/>
        <rect width="1280" height={TOTAL_H} fill="url(#wf-b-grid)" opacity="0.65"/>

        <g filter="url(#wf-b-sk)">
          {/* ── LEFT NAV RAIL ─────────────────────────────────────── */}
          ...
          <rect x={COL_X+16} y={14} width={260} height={18} rx="1" fill="#c8c4bc" opacity="0.8"/>
          ...
          {[
            ...
            { h: 210, content: "wireframes" },
            { h: 220, content: "decisions" },
            { h: 215, content: "screens" },
            { h: 175, content: "findings" },
              ...
              <g key={idx}>
                ...
                {content === "text" && (
                  <>
                    ...
                    <Ln x={COL_X+20} y={y+58} w={COL_W-80}/>
                    <Ln x={COL_X+20} y={y+70} w={COL_W-100}/>
                    <Ln x={COL_X+20} y={y+82} w={COL_W-180} op={0.6}/>
                  </>
                )}
                {content === "text+quote" && (
                  <>
                    <Ln x={COL_X+20} y={y+46} w={COL_W-40}/>
                    <Ln x={COL_X+20} y={y+58} w={COL_W-80}/>
                    <rect x={COL_X+20} y={y+78} width={3} height={60} fill="#c8c4bc"/>
                    <Ln x={COL_X+32} y={y+84} w={560} op={0.7}/>
                    <Ln x={COL_X+32} y={y+96} w={480} op={0.7}/>
                    <Ln x={COL_X+32} y={y+108} w={520} op={0.7}/>
                    <Ln x={COL_X+32} y={y+120} w={360} op={0.5}/>
                  </>
                )}
                {content === "highlight" && (
                  <>
                    <rect x={COL_X+20} y={y+42} width={COL_W-40} height={58} rx="2" fill="#e8e4de" stroke="#c8c4bc" strokeWidth="1"/>
                    <Ln x={COL_X+36} y={y+54} w={560}/>
                    <Ln x={COL_X+36} y={y+66} w={420}/>
                    <text x={COL_X+36} y={y+90} fontSize="7.5" fill="#9ca3af" fontFamily="monospace">[ key insight ]</text>
                  </>
                )}
                {content === "wireframes" && (
                  <>
                    <Ln x={COL_X+20} y={y+46} w={COL_W-80}/>
                    <Ln x={COL_X+20} y={y+58} w={COL_W-120}/>
                    {[0,1,2].map(i=><Xbox key={i} x={COL_X+20+i*(COL_W-60)/3+8} y={y+75} w={(COL_W-60)/3-16} h={110}/>)}
                    <text x={COL_X+110} y={y+196} fontSize="7" fill="#9ca3af" fontFamily="monospace">option A</text>
                    <text x={COL_X+455} y={y+196} fontSize="7" fill="#9ca3af" fontFamily="monospace">option B</text>
                    <text x={COL_X+800} y={y+196} fontSize="7" fill="#9ca3af" fontFamily="monospace">chosen ✓</text>
                  </>
                )}
                {content === "decisions" && (
                  <>
                    {[0,1,2,3].map(i=>(
                      <g key={i}>
                        <Ln x={COL_X+20} y={y+46+i*42} w={48} op={0.5}/>
                        <Ln x={COL_X+78} y={y+46+i*42} w={380}/>
                        <Ln x={COL_X+78} y={y+58+i*42} w={COL_W-120} op={0.65}/>
                        <Ln x={COL_X+78} y={y+70+i*42} w={COL_W-200} op={0.45}/>
                      </g>
                    ))}
                  </>
                )}
                {content === "screens" && (
                  <>
                    <Xbox x={COL_X+20} y={y+42} w={(COL_W-52)/3} h={155}/>
                    <Xbox x={COL_X+36+(COL_W-52)/3} y={y+42} w={(COL_W-52)/3} h={155}/>
                    <Xbox x={COL_X+52+(COL_W-52)*2/3} y={y+42} w={(COL_W-52)/3} h={155}/>
                  </>
                )}
                {content === "findings" && (
                  <>
                    {[0,1,2,3].map(i=>(
                      <g key={i}>
                        <rect x={COL_X+20} y={y+46+i*30} width={7} height={7} rx="1" fill="#c8c4bc"/>
                        <Ln x={COL_X+36} y={y+46+i*30} w={560}/>
                        <Ln x={COL_X+36} y={y+58+i*30} w={380} op={0.6}/>
                      </g>
                    ))}
                  </>
                )}
                {content === "metrics" && (
                  <>
                    {[0,1,2,3].map(i=>(
                      <g key={i}>
                        <rect x={COL_X+20+i*((COL_W-40)/4)} y={y+42} width={(COL_W-60)/4} height={70} rx="2" fill="#f0ede8" stroke="#c8c4bc" strokeWidth="1"/>
                        <rect x={COL_X+36+i*((COL_W-40)/4)} y={y+54} width={90} height={16} rx="1" fill="#c8c4bc"/>
                        <Ln x={COL_X+36+i*((COL_W-40)/4)} y={y+78} w={120} op={0.5}/>
                      </g>
                    ))}
                  </>
                )}
          ...
          <text x={1130} y={TOTAL_H-12} fontSize="8" fill="#c8c4bc" fontFamily="monospace">Layout B · Side Nav</text>
        </g>
      </svg>
    </div>
  );
